...
#pragma config(Sensor, dgtl7,  xAxisLimit,     sensorTouch)
#pragma config(Sensor, dgtl8,  yAxisLimit,     sensorTouch)
...
void moveToPoint(int x2, int y2) {
	...
	motor[xAxis] = (delta_x * 127) / (XAXISMAX / scale);
	motor[yAxis] = (delta_y * 127) / (YAXISMAX / scale);
